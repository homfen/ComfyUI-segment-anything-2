...
class SAM2VideoPredictor(SAM2Base):
    ...
    def add_new_points_or_box(
        ...
        if normalize_coords:
            ...
            video_points = torch.tensor([video_W, video_H]).to(points.device)
            print("add_new_points: ",
                  points.shape,
                  video_points.shape,
                  flush=True)
            if points.shape[2] > 0:
    ...
    def _add_output_per_object(self, inference_state, frame_idx, current_out,
        ...
        for obj_idx, obj_output_dict in output_dict_per_obj.items():
            ...
            obj_out = {
                ...
                None,
                "pred_masks":
                current_out["pred_masks"][obj_slice],
                "obj_ptr":
                current_out["obj_ptr"][obj_slice],
                "object_score_logits":
                current_out["object_score_logits"][obj_slice],
            }
    ...
    @torch.inference_mode()
    def clear_all_prompts_in_frame(self,
                                   inference_state,
                                   frame_idx,
                                   obj_id,
                                   need_output=True):
    ...
    def remove_object(self,
        ...
        for frame_idx in obj_input_frames_inds:
            self.clear_all_prompts_in_frame(inference_state,
                                            frame_idx,
                                            obj_id,
                                            need_output=False)
        ...
        def _slice_state(output_dict, storage_key):
            for frame_idx, out in output_dict[storage_key].items():
                ...
                self._add_output_per_object(inference_state, frame_idx, out,
                                            storage_key)
        ...
        if need_output:
            ...
            for frame_idx in obj_input_frames_inds:
                is_cond = any(
                    frame_idx in obj_temp_output_dict["cond_frame_outputs"] for
                    obj_temp_output_dict in temp_output_dict_per_obj.values())
                consolidated_out = self._consolidate_temp_output_across_obj(
                    inference_state,
                    frame_idx,
                    is_cond=is_cond,
