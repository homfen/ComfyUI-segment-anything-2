...
class DownloadAndLoadSAM2Model:
    ...
    def loadmodel(self, model, segmentor, device, precision):
        ...
        model = load_model(model_path, model_cfg_path, segmentor, dtype,
                           device)
...
class Sam2Segmentation:
    ...
    def segment(self,
        ...
        if segmentor == 'video' and bboxes is not None and "2.1" not in sam2_model[
                "version"]:
            raise ValueError(
                "2.0 model doesn't support bboxes with video segmentor")

        if segmentor == 'video':  # video model needs images resized first thing
            model_input_image_size = model.image_size
            print("Resizing to model input image size: ",
                  model_input_image_size)
            image = common_upscale(image.movedim(-1,
                                                 1), model_input_image_size,
                                   model_input_image_size, "bilinear",
        ...
        if coordinates_positive is not None:
            try:
                ...
                coordinates_positive = [(coord['x'], coord['y'])
                ...
                if coordinates_negative is not None:
                    ...
                    coordinates_negative = [(coord['x'], coord['y'])
                                            for coord in coordinates_negative]
            except:
                pass

            if not individual_objects:
                positive_point_coords = np.atleast_2d(
                    np.array(coordinates_positive))
            else:
            ...
            if coordinates_negative is not None:
                ...
                if individual_objects:
                    assert negative_point_coords.shape[
                        0] <= positive_point_coords.shape[
                            0], "Can't have more negative than positive points in individual_objects mode"
                    if negative_point_coords.ndim == 2:
                        negative_point_coords = negative_point_coords[:, np.
                                                                      newaxis, :]
                    # Extend negative coordinates to match the number of positive coordinates
                    while negative_point_coords.shape[
                            0] < positive_point_coords.shape[0]:
                        negative_point_coords = np.concatenate(
                            (negative_point_coords,
                             negative_point_coords[:1, :, :]),
                            axis=0)
        ...
        # After handling bboxes, before the video segmentor section
        if coordinates_positive is None and bboxes is not None:
            final_coords = None
            final_labels = None
            ...
            if coordinates_negative is not None:
                if not individual_objects:
                    negative_point_labels = np.zeros(
                        len(negative_point_coords))  # 0 = negative
        ...
        with torch.autocast(
                mm.get_autocast_device(device),
                dtype=dtype) if autocast_condition else nullcontext():
            if segmentor == 'single_image':
                ...
                for i in range(len(image_np)):
                    ...
                    out_masks, scores, logits = model.predict(
                        ...
                        if coordinates_positive is not None else None,
                        point_labels=final_labels
                        if coordinates_positive is not None else None,
                        box=input_box,
                        multimask_output=True
                        if not individual_objects else False,
                    ...
                    if out_masks.ndim == 3:
                        ...
                        out_masks = out_masks[sorted_ind][
                            0]  #choose only the best result for now
                ...
                self.inference_state = model.init_state(image.permute(
                    0, 3, 1, 2).contiguous(),
                                                        H,
                                                        W,
                                                        device=device)
                ...
                if individual_objects and bboxes is not None:
                    raise ValueError(
                        "bboxes not supported with individual_objects")

                if individual_objects:
                    for i, (coord,
                            label) in enumerate(zip(final_coords,
                                                    final_labels)):
        ...
        if not keep_model_loaded:
            ...
            if hasattr(self, 'inference_state'
                       ) and self.inference_state is not None and hasattr(
                           model, "reset_state"):
                model.reset_state(self.inference_state)
...
class Sam2VideoSegmentationAddPoints:
    ...
    def segment(self,
        ...
        positive_point_labels = [1] * len(
            positive_point_coords)  # 1 = positive
        positive_point_labels = np.array(positive_point_labels)
        print("positive coordinates: ", positive_point_coords)

        if coordinates_negative is not None:
            negative_point_coords = np.array(coordinates_negative)
            negative_point_labels = [0] * len(
                negative_point_coords)  # 0 = negative
        ...
        autocast_condition = not mm.is_device_mps(device)
        with torch.autocast(
                mm.get_autocast_device(model.device),
                dtype=dtype) if autocast_condition else nullcontext():
...
class Sam2AutoSegmentation:
    ...
    def segment(self, image, sam2_model, points_per_side, points_per_batch,
                ...
                mask_threshold, crop_nms_thresh, crop_overlap_ratio,
                keep_model_loaded):
        offload_device = mm.unet_offload_device()
        model = sam2_model["model"]
        device = sam2_model["device"]
        ...
        with torch.autocast(
            ...
            for img_np in image_np:
                ...
                colors = [
                    tuple(random.choices(range(256), k=3))
                ...
                for mask, color in zip(mask_list, colors):
                    ...
                    overlay_image = np.where(colored_mask > 0, colored_mask,
                                             overlay_image)
        ...
        return (mask_tensor.cpu().float(), segment_image_tensor.cpu().float(),
                bbox_list)
